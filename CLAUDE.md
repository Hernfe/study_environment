...
5. `npm run build`. The build ends with the build report (`scripts/lint_questions.py`): fix every finding for the new lecture (option lengths, answer positions, question mix, required fields, word banks, plain-text maths). Update `CREDITS.md` (videos and posters too), commit.
