...
const SYNAPSE_REGIONS = [
  ...
  { id: 'vesicles', label: 'Synaptic vesicles', body: 'About 50 nm across, filled with transmitter. Dozens per terminal, clustered near the active zones.', x: 46, y: 58, w: 12, h: 10, mx: 42, my: 60, side: 'left' },
  { id: 'granules', label: 'Secretory granules', body: 'Larger, about 100 nm, with a dense protein core. They store peptide transmitters and release them away from the active zones.', x: 35, y: 55, w: 6, h: 8, mx: 34, my: 56, side: 'left' },
  ...
  { id: 'active-zone', label: 'Active zone', body: 'The presynaptic membrane specialisation where docked vesicles fuse. The voltage-gated Ca2+ channels sit here.', shape: 'rect', x: 48, y: 63, w: 22, h: 4, mx: 57, my: 63, side: 'right' },
  { id: 'cleft', label: 'Synaptic cleft', body: 'The 20 to 50 nm gap, ten times the gap at a gap junction, filled with extracellular protein that glues the two membranes together.', shape: 'line', x: 32, y: 66.5, x2: 62, y2: 66.5, mx: 34, my: 66.5, side: 'left' },
...
const EM_REGIONS = [
  ...
  { id: 'vesicles', label: 'Synaptic vesicles', body: 'The many small circles, each about 50 nm across.', x: 62, y: 58, w: 12, h: 12, mx: 66, my: 58, side: 'right' },
...
const CYCLE_REGIONS = [
  ...
  { id: 'ca', label: 'Voltage-gated calcium channel', body: 'The action potential opens it. Ca2+ floods in down a huge gradient (0.0002 mM inside).', x: 35, y: 66, w: 5, h: 10, mx: 35, my: 68, side: 'bottom' },
...
const SNARE_REGIONS = [
  ...
  { id: 'synaptotagmin', label: 'Synaptotagmin', body: 'The vesicle protein that senses Ca2+ (blue). Ca2+ binding triggers fusion.', x: 5, y: 26, w: 6, h: 8, mx: 4, my: 24, side: 'left' },
  { id: 'channel', label: 'Calcium channel', body: 'Right next to the docked vesicle, so the local Ca2+ rise is fast and large.', x: 29, y: 82, w: 5, h: 12, mx: 29, my: 90, side: 'bottom' },
  { id: 'ca-entry', label: 'Ca2+ enters', body: 'The middle panel: Ca2+ (red dots) pours in and binds synaptotagmin; the SNAREs are zipped and the membranes are pulled together.', x: 60, y: 62, w: 10, h: 14, mx: 62, my: 66, side: 'bottom' },
...
const GPCR_REGIONS = [
  ...
  { id: 'channel', label: 'G-protein-gated ion channel (shortcut pathway)', body: 'Effector type 1: the G-protein opens a separate ion channel directly. Fastest metabotropic route, 30 to 100 ms.', x: 38, y: 32, w: 10, h: 28, mx: 38, my: 26, side: 'top' },
...
const GABAA_REGIONS = [
  { id: 'gaba', label: 'GABA binding site', body: 'GABA (the green ball) binds at the top of the channel and opens the Cl- pore.', x: 49, y: 30, w: 8, h: 8, mx: 49, my: 28, side: 'top' },
  ...
  { id: 'pore', label: 'Cl- pore', body: 'The channel is selective for Cl-. Opening it pulls Vm toward ECl, about -65 mV.', shape: 'rect', x: 49, y: 62, w: 6, h: 20, mx: 49, my: 66, side: 'bottom' },
];
...
const NE_REGIONS = [
  ...
  { id: 'channel', label: '5 K+ channel phosphorylated and closed', body: 'PKA phosphorylates a dendritic K+ channel, which closes. Lower gK raises the membrane resistance and the length constant.', x: 82, y: 35, w: 12, h: 24, mx: 82, my: 22, side: 'top' },
];
...
export default {
  ...
  objectives: [
    'Trace chemical synaptic transmission from an action potential reaching the axon terminal through Ca2+-dependent release, receptor activation, and transmitter removal.',
  ...
  sections: [
    // 1 -----------------------------------------------------------------
    {
      ...
      keyTerms: ['spike-initiation zone', 'saltatory conduction', 'voltage-gated Ca2+ channel'],
      blocks: [
        ...
        { type: 'steps', title: 'From spike to release', steps: [
          { title: 'Start.', body: 'The spike-initiation zone has a high density of voltage-gated Na+ channels: the axon hillock in a central neuron, the sensory ending in a sensory neuron.' },
          { title: 'Waveform.', body: 'Rapid Na+ channel opening gives the rising phase; Na+ inactivation and delayed K+ opening repolarize; continued K+ current gives the undershoot. Equilibrium potentials: EK -80, ENa +62, ECa +123, ECl -65 mV.' },
          ...
          { title: 'Arrival.', body: 'The spike depolarizes the terminal, opens voltage-gated Ca2+ channels, and Ca2+ entry triggers vesicle fusion and release. That is where this lecture begins.' },
        ] },
        { type: 'whyItMatters', body: 'Everything below is what happens in the roughly one millisecond between Ca2+ entering the terminal and the next cell responding.' },
      ],
      conceptQuiz: [
        {
          id: 'recap-1',
          ...
          options: [
            { text: 'Depolarizes it and opens voltage-gated Ca2+ channels', feedback: 'Correct. Ca2+ entry is the release signal.' },
            { text: 'Opens transmitter-gated channels on the terminal', feedback: 'Transmitter-gated channels are on the postsynaptic side.' },
            { text: 'Pumps transmitter across the cleft', feedback: 'Transmitter leaves by exocytosis and diffuses; nothing pumps it across.' },
          ],
          correct: 0,
        },
      ],
    ...
    // 2 -----------------------------------------------------------------
    {
      ...
      blocks: [
        ...
        { type: 'keyNumber', items: [
          { value: '20 to 50 nm', label: 'width of the synaptic cleft, ten times the gap at a gap junction' },
          { value: '50 nm', label: 'diameter of a synaptic vesicle; secretory granules are about 100 nm' },
        ] },
        { type: 'steps', title: 'Chemical transmission in one breath', steps: [
          'The action potential depolarizes the terminal and opens voltage-gated Ca2+ channels at the active zones.',
          'Ca2+ enters and triggers synaptic vesicles to fuse with the presynaptic membrane: exocytosis.',
      ...
      conceptQuiz: [
        ...
        },
        {
          ...
          options: [
            { text: 'Axosomatic', feedback: 'Correct.' },
        ...
        },
        {
          ...
          options: [
            { text: 'Secretory granules, about 100 nm with a dense core', feedback: 'Correct.' },
            { text: 'Synaptic vesicles, about 50 nm', feedback: 'These hold amino acid and amine transmitters.' },
            { text: 'Mitochondria', feedback: 'Mitochondria make ATP.' },
          ],
          correct: 0,
        },
      ],
    ...
    // 3 -----------------------------------------------------------------
    {
      ...
      blocks: [
        ...
        { type: 'steps', title: 'What an electrical synapse does (slide 11)', steps: [
          ...
          'Cell 2 shows an electrical PSP: about 1 mV, from -65 to -64 mV, followed by a brief dip below rest. Mind the y scale: it is tiny next to the spike.',
        ...
        figureBlock(
          hotspots('electrical-psp', 1022 / 472, 'Left: an electron micrograph and a drawing of two dendrites joined by a gap junction. Right: the action potential in cell 1 above the 1 mV electrical PSP in cell 2.', [
            ...
            { id: 'spike', label: 'Action potential in cell 1', body: 'A full spike of about 100 mV.', x: 72, y: 24, w: 30, h: 36, mx: 72, my: 6, side: 'top' },
            { id: 'psp', label: 'Electrical PSP in cell 2', body: 'About 1 mV, then a small dip below rest. Note the scale: -63 to -65 mV.', x: 72, y: 74, w: 30, h: 36, mx: 72, my: 94, side: 'bottom' },
          ], { quiz: false, gutter: 'ends', layout: 'stack' }),
        ...
        demoBlock('synapse-compare', {
          types: {
            chemical: { label: 'Chemical synapse', delay: 0.5, amplitude: 4, tauRise: 0.4, tauDecay: 3, bidirectional: false, body: 'Release, diffusion and binding add a delay of about 0.5 to 1 ms. One way only.', responseNote: 'an EPSP of a few millivolts', noneNote: 'the postsynaptic cell has no vesicles facing the presynaptic one, so nothing comes back' },
            electrical: { label: 'Electrical synapse', delay: 0.05, amplitude: 1, tauRise: 0.2, tauDecay: 1, undershoot: 0.4, bidirectional: true, body: 'Current flows straight through the gap junction with almost no delay. Both directions.', responseNote: 'an electrical PSP of about 1 mV with a small dip after it' },
          },
          labels: { type: 'Synapse', stimulate: 'Stimulate', cell1: 'cell 1', cell2: 'cell 2', vm: 'Vm of', delay: 'delay', response: 'Response in the other cell', none: 'no response', direction: 'Direction', oneWay: 'one way, presynaptic to postsynaptic', bothWays: 'both ways', after: 'after' },
        },
      ...
      conceptQuiz: [
        {
          ...
          options: [
            { text: 'Twelve: six per connexon, two connexons', feedback: 'Correct.' },
            { text: 'Six', feedback: 'Six make one connexon, which is only half a channel.' },
            { text: 'Two', feedback: 'Two connexons, each made of six connexins.' },
          ],
          correct: 0,
        },
        {
          ...
          options: [
            ...
            { text: 'They stopped oscillating', feedback: 'The oscillations are intrinsic to each cell.' },
            { text: 'They fired faster', feedback: 'Firing rate was not the change; timing was.' },
          ],
          correct: 0,
        },
        {
          ...
          options: [
            { text: 'About 1 mV or less', feedback: 'Correct. Many must sum to fire the cell.' },
            { text: 'About 20 mV', feedback: 'Far too large; look at the y scale on the slide.' },
      ...
      conceptQuiz: [
        {
          ...
          options: [
            { text: 'An axospinous synapse, a kind of axodendritic synapse', feedback: 'Correct.' },
            { text: 'An axosomatic synapse', feedback: 'That is on the soma.' },
            { text: 'A dendrodendritic synapse', feedback: 'The presynaptic side here is an axon.' },
          ],
          correct: 0,
        },
        {
          ...
          options: [
            { text: 'How many release sites it has: larger synapses have more', feedback: 'Correct.' },
            { text: 'Which transmitter it uses', feedback: 'Active zones look alike whatever the transmitter.' },
            { text: 'Whether it is electrical', feedback: 'Electrical synapses have gap junctions, not active zones.' },
          ],
          correct: 0,
        },
      ],
      ...
      conceptQuiz: [
        {
          ...
          options: [
            { text: 'The protein-rich region holding the transmitter receptors', feedback: 'Correct.' },
            { text: 'The vesicle cluster in the bouton', feedback: 'Vesicles are presynaptic.' },
            { text: 'The number of spines per micrometre', feedback: 'That is a spine density, a different thing.' },
          ],
          correct: 0,
        },
        {
          ...
          options: [
            { text: 'The three-dimensional shape of dendrites and spines and where their contacts are', feedback: 'Correct.' },
            { text: 'The size of the EPSP at each synapse', feedback: 'That needs an electrical recording.' },
            { text: 'Which transmitter each terminal releases', feedback: 'Structure alone does not name the transmitter.' },
          ],
          correct: 0,
        },
      ],
      ...
      blocks: [
        ...
        figureBlock(
          hotspots('synthesis-inset', 325 / 356, 'The terminal route: a precursor molecule is converted by a synthesizing enzyme, and a transporter protein loads the transmitter into a vesicle.', [
            ...
            { id: 'vesicle', label: 'Loaded synaptic vesicle', body: 'Stored until Ca2+ arrives.', x: 50, y: 84, w: 34, h: 28, mx: 50, my: 90, side: 'bottom' },
        ...
        { type: 'misconception', wrong: 'The chemical family of a transmitter tells you its effect.', right: 'The receptor decides. ACh slows the heart through a G-protein-coupled receptor that opens K+ channels, and excites skeletal muscle through a Na+-permeable transmitter-gated channel.' },
      ],
      conceptQuiz: [
        {
          id: 'transmitters-1',
          ...
          options: [
            { text: 'In the soma, on the rough ER, then processed in the Golgi', feedback: 'Correct. They travel to the terminal in secretory granules.' },
            { text: 'In the axon terminal cytosol', feedback: 'That is where amines and amino acids are made.' },
            { text: 'Inside the synaptic vesicle', feedback: 'Vesicles are loaded by transporters; they do not synthesize.' },
          ],
          correct: 0,
        },
        {
    ...
    // 7 -----------------------------------------------------------------
    {
      ...
      blocks: [
        figureBlock(
          hotspots('vesicle-cycle', 1049 / 583, 'A terminal above a cleft: a loaded vesicle, a Ca2+ channel with Ca2+ entering, a vesicle fusing and spilling transmitter, and a recycled vesicle.', CYCLE_REGIONS, { gutter: 'all' }),
        ...
        { type: 'steps', title: 'From spike to release', steps: [
          { title: 'Ca2+ channels open.', body: 'The spike depolarizes the terminal; voltage-gated Ca2+ channels at the active zone open. The driving force on Ca2+ is huge because cytosolic Ca2+ is only 0.0002 mM.' },
          { title: 'A local Ca2+ microdomain forms.', body: 'Next to the open channels Ca2+ rises above about 0.01 mM. The vesicles that respond fastest are those already docked there.' },
          { title: 'Synaptotagmin senses Ca2+.', body: 'The vesicle protein binds Ca2+ and triggers fusion of the SNARE-docked vesicle. A fusion pore opens and widens until the vesicle membrane is part of the presynaptic membrane.' },
          { title: 'Recycle.', body: 'Endocytosis retrieves the membrane; the vesicle is refilled. Under prolonged firing a reserve pool held on the cytoskeleton is mobilised, also by Ca2+.' },
        ] },
        figureBlock(
          hotspots('snare', 1357 / 360, 'Three panels: a docked vesicle held by SNAREs beside a calcium channel; Ca2+ entering and binding synaptotagmin; the vesicle fused and releasing.', SNARE_REGIONS, { gutter: 'all', layout: 'stack' }),
          'How to SNARE a vesicle (textbook Box 5.3). v-SNAREs on the vesicle bind t-SNAREs on the terminal membrane; synaptotagmin is the Ca2+ sensor.',
          'Left: a vesicle hangs above a membrane, tethered by green and blue proteins to red and purple proteins; a pink channel sits in the membrane. Middle: red Ca2+ dots pour in and the proteins are zipped together. Right: the vesicle has opened into the membrane and dots stream out.'
        ),
        { type: 'keyNumber', items: [
          { value: '0.2 ms', label: 'from Ca2+ influx to exocytosis in the squid giant synapse; about 60 us in mammals at body temperature' },
          { value: '1 quantum', label: 'the transmitter in one vesicle. A CNS synapse often releases one per spike (an EPSP of tenths of a millivolt); the neuromuscular junction about 200 (40 mV)' },
        ] },
        demoBlock('synapse-timeline', {
          steps: [
            ...
            { key: 'calcium', label: 'Ca2+ enters', body: 'Voltage-gated Ca2+ channels at the active zone open; Ca2+ floods the microdomain around the docked vesicles.', time: 0.1, timeNote: 'the channels open within the falling phase of the spike' },
            { key: 'fusion', label: 'Synaptotagmin triggers SNARE-mediated fusion', body: 'Ca2+ binds synaptotagmin; the zipped SNAREs pull the membranes together and a fusion pore opens.', time: 0.2, timeNote: 'about 60 microseconds after Ca2+ entry in a mammal at body temperature' },
            { key: 'release', label: 'Release and diffusion', body: 'The vesicle contents, one quantum, spill into the cleft and diffuse the 20 to 50 nm to the other side.', time: 0.3 },
            { key: 'binding', label: 'Receptor binding', body: 'Transmitter binds receptors in the postsynaptic density. A transmitter-gated channel opens within microseconds; the postsynaptic current begins.', time: 0.5, timeNote: 'the synaptic delay from spike to response is about 0.5 to 1 ms' },
        ...
        'A schematic terminal above a cleft and a postsynaptic membrane with three receptors. As the step slider moves, the Ca2+ channels fill, vesicles are highlighted, one fuses and dots cross to the receptors, then fade.'),
        { type: 'example', title: 'Yeast and a billion years', body: 'The same SNARE machinery drives secretion in yeast. Membrane fusion is a general cellular problem, and the proteins have been conserved for more than a billion years; the synapse adds fast, precise Ca2+ control. Botulinum toxins cut specific SNAREs and so block release at the neuromuscular junction.' },
        { type: 'detail', title: 'Peptide release', body: 'Secretory granules also release by Ca2+-dependent exocytosis, but away from the active zones. Ca2+ has to build up through the whole terminal, which takes high-frequency trains of spikes, and release is slow, 50 ms or more.' },
      ],
      conceptQuiz: [
        {
          id: 'release-1',
          prompt: 'Which protein is the Ca2+ sensor for fast release?',
          options: [
            { text: 'Synaptotagmin, on the vesicle', feedback: 'Correct.' },
            { text: 'The t-SNARE', feedback: 'SNAREs dock the vesicle; they do not sense Ca2+.' },
            { text: 'Kinesin', feedback: 'Kinesin is the transport motor.' },
          ],
          correct: 0,
        },
        {
          id: 'release-2',
          prompt: 'Why can release happen within a fraction of a millisecond of Ca2+ entry?',
          options: [
            { text: 'The Ca2+ channels sit next to vesicles that are already docked at the active zone', feedback: 'Correct. The local Ca2+ rise is fast and large.' },
            { text: 'Ca2+ diffuses through the whole terminal instantly', feedback: 'It does not; only the microdomain near the channels rises quickly.' },
            { text: 'Vesicles are made on demand', feedback: 'They are made and loaded in advance.' },
          ],
          correct: 0,
        },
        {
          ...
          options: [
            { text: 'The contents of one synaptic vesicle', feedback: 'Correct. An EPSP is a whole-number multiple of it.' },
            { text: 'One molecule of transmitter', feedback: 'A vesicle holds several thousand molecules.' },
            { text: 'The amount released per second', feedback: 'It is a packet, not a rate.' },
          ],
          correct: 0,
        },
      ],
      ...
      blocks: [
        ...
        ),
        {
          ...
          rows: [
            ...
            { label: 'Effect', cells: ['EPSP if the pore passes Na+ (ACh, glutamate); IPSP if it passes Cl- (GABA, glycine)', 'Slower, longer, more diverse: modulation of channels and metabolism'] },
          ],
        },
        figureBlock(
      ...
      conceptQuiz: [
        {
          ...
          options: [
            { text: 'The receptor itself is an ion channel that opens when transmitter binds', feedback: 'Correct.' },
            { text: 'It activates a G-protein', feedback: 'That is metabotropic.' },
            { text: 'It pumps ions using ATP', feedback: 'Receptors do not pump.' },
          ],
          correct: 0,
        },
        {
          ...
          options: [
            { text: 'Acetylcholinesterase in the cleft breaks it down', feedback: 'Correct.' },
            { text: 'Reuptake into the terminal as intact ACh', feedback: 'ACh is cleaved first; choline is taken up.' },
            { text: 'It is not removed; the receptors desensitize', feedback: 'Desensitization is what happens when removal fails.' },
          ],
          correct: 0,
        },
      ],
    ...
    // 9 -----------------------------------------------------------------
    {
      ...
      blocks: [
        { type: 'definition', term: 'EPSP', body: 'Excitatory postsynaptic potential: a brief depolarization from about -65 mV that brings the membrane toward threshold. On the slide, transmitter opens channels and Na+ enters (ENa = +62 mV).' },
        { type: 'definition', term: 'IPSP', body: 'Inhibitory postsynaptic potential: a brief hyperpolarization. On the slide, transmitter opens Cl- channels and Cl- enters, pulling Vm toward ECl = -65 mV. Inward negative charge is the same as outward positive current.' },
        figureBlock(
          hotspots('psp-generation', 1208 / 684, 'Four panels: Na+ entering through transmitter-gated channels with an EPSP trace; Cl- entering through transmitter-gated channels with an IPSP trace.', [
            { id: 'na', label: 'Na+ enters', body: 'Transmitter-gated channels open; Na+ flows in down its electrochemical gradient.', x: 22, y: 25, w: 40, h: 40, mx: 8, my: 25, side: 'left' },
            { id: 'epsp', label: 'EPSP', body: 'Vm rises a few millivolts from -65 mV and decays over several milliseconds.', x: 74, y: 25, w: 40, h: 40, mx: 92, my: 25, side: 'right' },
            { id: 'cl', label: 'Cl- enters', body: 'Transmitter-gated Cl- channels open; Cl- flows in because Vm is above ECl.', x: 22, y: 75, w: 40, h: 40, mx: 8, my: 75, side: 'left' },
            { id: 'ipsp', label: 'IPSP', body: 'Vm dips toward ECl and recovers.', x: 74, y: 75, w: 40, h: 40, mx: 92, my: 75, side: 'right' },
        ...
        { type: 'equation', title: 'Synaptic current', items: [
          { expression: 'I_syn = g_syn * (Vm - E_rev)', label: 'g_syn from the open transmitter-gated channels; E_rev is the reversal potential, where the current changes direction. Zero at E_rev, however many channels are open.' },
        ], note: 'A channel permeable to both Na+ and K+ (ACh, AMPA) reverses near 0 mV. At -65 mV the driving force is -65 mV: inward current, EPSP.' },
        demoBlock('driving-force', {
          threshold: -55,
          channels: [
            { key: 'mixed', label: 'Na+ and K+ (ACh, AMPA)', erev: 0, body: 'Equally permeable to Na+ and K+, so E_rev is about 0 mV, far above threshold.' },
            { key: 'na', label: 'Na+ only', erev: 62, body: 'E_rev = ENa = +62 mV. Strongly excitatory.' },
            { key: 'cl', label: 'Cl- (GABA-A, glycine)', erev: -65, body: 'E_rev = ECl = -65 mV, at rest and below threshold. Inhibitory.' },
            { key: 'k', label: 'K+ only (GABA-B via G-protein)', erev: -80, body: 'E_rev = EK = -80 mV. Inhibitory: outward K+ current at any Vm above -80.' },
          ],
          labels: { channel: 'Channel selectivity', vm: 'Membrane potential', current: 'Current', currentUnit: 'pA', inward: 'inward (cations in), depolarising', outward: 'outward, hyperpolarising', none: 'zero', drivingForce: 'Driving force', reversal: 'E_rev', threshold: 'threshold', vmAxis: 'Vm (mV)', traceTitle: 'change in Vm', effect: 'Synapse', epsp: 'EPSP', ipsp: 'IPSP', noChange: 'no voltage change', epspWeak: 'small depolarisation, toward E_rev', ipspToward: 'repolarisation toward E_rev', excitatory: 'excitatory: E_rev above threshold', inhibitory: 'inhibitory: E_rev at or below threshold', epspNote: 'The current takes Vm toward E_rev, past threshold if enough channels open.', ipspNote: 'The current takes Vm down toward E_rev, away from threshold.', noneNote: 'Vm is at E_rev, so no net current, but the open channels still raise the conductance (shunting).', epspWeakNote: 'Below E_rev the current is inward, but it can only bring Vm up to E_rev, which is below threshold. Still inhibitory in effect.', ipspTowardNote: 'Above E_rev the mixed channel carries outward current, which pulls Vm back down toward 0 mV.' },
        },
        'Driving force explorer. Choose the channel and drag Vm. Left: the I-V line for that channel with the current at Vm marked. Right: the resulting change in Vm. Excitatory or inhibitory depends on where E_rev sits relative to threshold, not on the ion alone.',
        ...
        { type: 'misconception', wrong: 'Inhibition always means hyperpolarization.', right: 'Inhibition means making a spike less likely. If Vm already equals ECl, opening Cl- channels causes no IPSP, but the extra conductance still shunts excitation. Inhibitory K+ channels pull toward EK.' },
        { type: 'detail', title: 'Reversal potential and the I-V plot (Box 5.4)', body: 'Measuring the synaptic current at several clamped voltages gives an I-V plot. The voltage where the current crosses zero is the reversal potential, which tells you which ions the channel passes. A transmitter is excitatory if it drives Vm toward a value above spike threshold and inhibitory if it drives Vm toward a value below threshold.' },
      ],
      conceptQuiz: [
        {
          id: 'psp-1',
          prompt: 'A transmitter opens channels with a reversal potential of 0 mV in a cell at -65 mV. The current is',
          options: [
            { text: 'Inward, producing an EPSP', feedback: 'Correct. I = g (Vm - E_rev) is negative: cations enter.' },
            { text: 'Outward, producing an IPSP', feedback: 'Outward current would need Vm above E_rev.' },
            { text: 'Zero, because the channels are mixed', feedback: 'Zero only at E_rev itself.' },
          ],
          correct: 0,
        },
        {
          id: 'psp-2',
          prompt: 'In the slide example ECl is -65 mV and the cell rests at -65 mV. GABA opens the Cl- channels. What happens to Vm?',
          options: [
            { text: 'Nothing visible, but the membrane conductance rises and excitation is shunted', feedback: 'Correct. No driving force, still inhibitory.' },
            { text: 'A large hyperpolarization', feedback: 'There is no driving force on Cl- at ECl.' },
            { text: 'A depolarization', feedback: 'Vm cannot move away from E_rev because of that channel.' },
          ],
          correct: 0,
        },
      ],
      ...
      blocks: [
        ...
        figureBlock(
          hotspots('patch-transmitter', 1601 / 615, 'The patch-clamp cartoon from lecture 2 above a current trace with downward steps while transmitter is applied.', [
            { id: 'patch', label: 'The patch-clamp set-up', body: 'The same method as for the voltage-gated Na+ channel in lecture 2, but now transmitter, not voltage, opens the channel.', shape: 'rect', x: 15, y: 12, w: 30, h: 22, mx: 15, my: 4, side: 'top' },
        ...
        { type: 'whyItMatters', body: 'The synaptic current is the sum of many such channels. When Vm is free to move, that inward current is what depolarizes the membrane and makes the EPSP; the clamped trace shows the current, not the EPSP.' },
        { type: 'detail', title: 'Not all transmitter-gated channels are Na+ channels', body: 'The slide title says Na+ channel, but transmitter-gated channels are less selective than voltage-gated ones: the ACh channel passes both Na+ and K+, which is why its reversal potential is near 0 mV rather than at ENa.' },
      ],
      conceptQuiz: [
        {
          id: 'recording-1',
          ...
          options: [
            ...
            { text: 'The membrane potential', feedback: 'Vm is clamped; the trace is current.' },
            { text: 'The EPSP amplitude', feedback: 'No EPSP can form while Vm is clamped.' },
          ],
          correct: 0,
        },
      ],
      ...
      blocks: [
        ...
        figureBlock(
          hotspots('summation', 1436 / 640, 'Three panels: one presynaptic axon and one EPSP; three axons active together and a larger EPSP; one axon firing three times and a staircase EPSP.', [
            ...
            { id: 'temporal', label: 'Temporal summation', body: 'EPSPs from the same synapse in quick succession, within about 1 to 15 ms, add up.', shape: 'rect', x: 90, y: 50, w: 16, h: 90, mx: 90, my: 6, side: 'top' },
          ], { quiz: false, gutter: 'ends', layout: 'stack' }),
        ...
        ),
        {
          ...
          rows: [
            ...
            { label: 'Temporal summation', cells: ['The same synapse', 'Fires again before the last EPSP has decayed (1 to 15 ms)'] },
          ],
        },
        { type: 'whyItMatters', body: 'The neuromuscular junction is fail-safe: one spike releases about 200 quanta and a 40 mV EPSP. A CNS synapse releases about one quantum, a few tenths of a millivolt, so many EPSPs must sum before the spike-initiation zone reaches threshold. That is what makes a neuron a computer rather than a relay.' },
      ],
      conceptQuiz: [
        {
          id: 'summation-1',
          prompt: 'Two EPSPs from the same synapse 5 ms apart add together. This is',
          options: [
            { text: 'Temporal summation', feedback: 'Correct.' },
            { text: 'Spatial summation', feedback: 'Spatial summation is across different synapses at the same time.' },
            { text: 'Shunting', feedback: 'Shunting reduces EPSPs; it does not add them.' },
          ],
          correct: 0,
        },
        {
          id: 'summation-2',
          prompt: 'A summed EPSP of 8 mV in the dendrite fails to fire the cell. Why might that be?',
          options: [
            ...
            { text: 'EPSPs cannot trigger spikes', feedback: 'They can, when summed to threshold at the initiation zone.' },
            { text: 'The dendrite has too many Na+ channels', feedback: 'Dendrites have few voltage-gated Na+ channels; that is not the problem.' },
          ],
          correct: 0,
        },
      ],
    ...
    // 12 ----------------------------------------------------------------
    {
      ...
      blocks: [
        ...
        figureBlock(
          hotspots('cable', 887 / 772, 'A dendritic cable with a current-injecting pipette and two recording pipettes, and a plot of depolarization against distance falling to 37 percent at one length constant.', [
            ...
            { id: 'lambda', label: 'Length constant lambda', body: 'The distance over which the depolarization falls to 37 percent of its value at the origin.', x: 62, y: 84, w: 8, h: 8, mx: 62, my: 84, side: 'right' },
          ], { quiz: false, gutter: 'all' }),
          'Decreasing depolarization along a long dendritic cable (textbook Figure 5.20).',
          'Above: a horizontal tube with a pipette injecting current in the middle and arrows leaking out through its wall; two small voltage traces, one large and one small. Below: a curve peaking at 100 percent under the pipette and falling to 37 percent at a distance marked lambda.'
        ),
        { type: 'equation', title: 'Length constant', items: [
          { expression: 'Vx = Vo / e^(x / lambda)    so at x = lambda, Vx = 0.37 Vo', label: 'Vo is the depolarization at the origin, x the distance, lambda the length constant.' },
        ], note: 'lambda rises with membrane resistance rm (fewer open channels, less leak) and falls with internal resistance ri (thin dendrite). ri is fixed by the dendrite; rm changes moment to moment with what other synapses are doing, so lambda is not constant.' },
        { type: 'whyItMatters', body: 'Dendrites are not purely passive: many carry voltage-gated Na+, Ca2+ and K+ channels that amplify distant EPSPs on their way to the soma, and in some cells let a somatic spike travel back into the dendrites. Effects go both ways.' },
        figureBlock(
          hotspots('shunting', 780 / 803, 'Two drawings of a dendrite with an excitatory synapse far out and an inhibitory synapse near the soma; with the inhibitory synapse inactive the somatic EPSP is present, with it active the somatic EPSP is gone.', [
            ...
            { id: 'inh-on', label: 'Inhibitory synapse, active', body: 'Bottom: its open Cl- channels shunt the current out of the dendrite, so the dendritic EPSP is still there but the somatic EPSP is gone.', x: 56, y: 62, w: 8, h: 16, mx: 56, my: 58, side: 'right' },
        ...
        { type: 'steps', title: 'How shunting works', steps: [
          'ECl sits near the resting potential and below threshold. Opening Cl- channels adds conductance and lowers membrane resistance, without much voltage change on its own.',
          'When excitation depolarizes that stretch of dendrite above ECl, Cl- enters and opposes the depolarization.',
        ...
        demoBlock('summation-shunt', {
          threshold: -55,
          labels: { synapses: 'Synapses active together', pulses: 'Spikes at each synapse', interval: 'Interval between spikes', shunt: 'Open Cl- channels near the soma (ECl = -65 mV)', peak: 'Peak somatic potential', threshold: 'Threshold', reached: 'reached', notReached: 'not reached', reachedNote: 'the neuron fires', notReachedNote: 'no spike', rest: 'rest', aboveRest: 'above rest', shuntOn: 'Cl- conductance open' },
        },
        'Summation and shunting. Add synapses (spatial summation) or spikes at one synapse (temporal summation) and watch the somatic potential approach the threshold line. Then open a Cl- conductance at ECl: the dashed line shows what the EPSP would have been. A simple passive model.',
      ...
      conceptQuiz: [
        {
          id: 'dendrites-1',
          prompt: 'What does a length constant of 1 mm mean?',
          options: [
            { text: 'A steady depolarization falls to 37 percent of its origin value 1 mm away', feedback: 'Correct.' },
            { text: 'The EPSP is gone after 1 mm', feedback: 'It is reduced to 37 percent, not gone.' },
            { text: 'The dendrite is 1 mm long', feedback: 'Lambda is about decay, not about length.' },
          ],
          correct: 0,
        },
        {
          ...
          options: [
            ...
            { text: 'Raises membrane resistance', feedback: 'More open channels mean less resistance.' },
            { text: 'Changes the internal resistance', feedback: 'Internal resistance depends on diameter and cytoplasm, not on channels.' },
          ],
          correct: 0,
        },
        {
          ...
          options: [
            ...
            { text: 'On the far dendritic tips', feedback: 'That only affects the inputs beyond it.' },
            { text: 'On the axon terminal', feedback: 'That would affect release, not this cell\'s firing.' },
          ],
          correct: 0,
        },
      ],
    ...
    // 13 ----------------------------------------------------------------
    {
      ...
      blocks: [
        ...
        figureBlock(
          hotspots('glutamate-receptors', 1293 / 742, 'Glutamate above three receptor channels in a membrane: AMPA, NMDA and kainate, each with its agonist named.', [
            ...
            { id: 'ampa', label: 'AMPA receptor', body: 'Named after its selective agonist AMPA. Passes Na+ and K+; fast EPSP.', x: 18, y: 60, w: 18, h: 40, mx: 12, my: 62, side: 'left' },
            { id: 'nmda', label: 'NMDA receptor', body: 'Named after NMDA. Passes Na+, K+ and Ca2+; blocked by Mg2+ at rest.', x: 50, y: 60, w: 18, h: 40, mx: 50, my: 92, side: 'bottom' },
        ...
        ),
        {
          ...
          rows: [
            { label: 'Fast excitation', cells: ['Glutamate', 'AMPA (and NMDA, kainate)', 'Na+ in, K+ out; net inward at negative Vm', 'Rapid EPSP'] },
            { label: 'Fast inhibition', cells: ['GABA (glycine for most of the rest)', 'GABA-A', 'Cl-', 'IPSP when Vm is above ECl; shunting either way'] },
          ],
        },
        figureBlock(
      ...
      conceptQuiz: [
        {
          id: 'systems-1',
          prompt: 'Why do AMPA channels depolarize the cell although they pass K+ as well as Na+?',
          options: [
            { text: 'At negative Vm more Na+ enters than K+ leaves, so the net current is inward', feedback: 'Correct. The reversal potential is near 0 mV.' },
            { text: 'They pass only Na+', feedback: 'They pass both; most also exclude Ca2+.' },
            { text: 'K+ is pumped back in', feedback: 'Pumps are irrelevant on this time scale.' },
          ],
          correct: 0,
        },
        {
          ...
          options: [
            { text: 'Bind a separate site and make openings more frequent when GABA is present', feedback: 'Correct.' },
            { text: 'Open the channel by themselves', feedback: 'Alone they do very little.' },
            { text: 'Block the Cl- pore', feedback: 'That would reduce inhibition; they enhance it.' },
          ],
          correct: 0,
        },
      ],
    ...
    // 14 ----------------------------------------------------------------
    {
      ...
      conceptQuiz: [
        ...
        },
        {
          ...
          options: [
            { text: 'GABA', feedback: 'Correct.' },
            { text: 'Glutamate', feedback: 'Glutamate is released by pyramidal neurons.' },
            { text: 'Norepinephrine', feedback: 'Norepinephrine comes from brainstem neurons, not cortical interneurons.' },
          ],
          correct: 0,
        },
      ],
    ...
    // 15 ----------------------------------------------------------------
    {
      ...
      keyTerms: ['Mg2+ block', 'coincidence detection', 'GABA-B receptor'],
      blocks: [
        { type: 'text', body: 'The NMDA receptor is the special snowflake among the glutamate channels: it is gated by transmitter and by voltage, and it lets Ca2+ in.' },
        figureBlock(
          hotspots('nmda', 463 / 540, 'Two NMDA receptors: at -65 mV glutamate is bound but a Mg2+ ion plugs the pore; at -30 mV the Mg2+ has left and Na+ and Ca2+ enter while K+ leaves.', [
            { id: 'blocked', label: '-65 mV: glutamate bound, pore blocked by Mg2+', body: 'The channel is open but a Mg2+ ion sits in the pore and little current passes.', x: 25, y: 45, w: 34, h: 50, mx: 25, my: 14, side: 'top' },
            { id: 'open', label: '-30 mV: depolarization expels Mg2+', body: 'Na+ and Ca2+ enter, K+ leaves. Net inward current and Ca2+ signalling.', x: 75, y: 45, w: 34, h: 50, mx: 75, my: 14, side: 'top' },
          ], { quiz: false, gutter: 'ends', layout: 'stack' }),
          'Inward current through the NMDA-gated channel (textbook Figure 6.21).',
          'Two pink channels side by side in a membrane. Left, labelled -65 mV: a green glutamate ball on top and a small grey Mg2+ ball stuck in the pore. Right, labelled -30 mV: the Mg2+ ball is outside, red arrows carry Na+ and Ca2+ in and K+ out.'
        ),
        { type: 'steps', title: 'Coincidence detection', steps: [
          'Glutamate binds the NMDA receptor and the pore opens, but at negative resting potentials Mg2+ is drawn into the pore and blocks it.',
          'Depolarization, usually from AMPA receptors at the same and neighbouring synapses, pops the Mg2+ out.',
          'With both conditions met, Na+ and Ca2+ enter and K+ leaves: inward current plus a Ca2+ signal.',
          'The entering Ca2+ activates enzymes and can change channels and gene expression: lasting change in the postsynaptic neuron, the basis of memory in Chapter 25.',
        ] },
        demoBlock('ampa-nmda', {
          labels: { vm: 'Membrane potential', glutamate: 'Glutamate bound', ampa: 'AMPA', nmda: 'NMDA', block: 'Mg2+ block', mg: 'Mg2+', currentUnit: 'pA', open: 'open, ions pass', blocked: 'open but blocked by Mg2+', closed: 'closed', outside: 'outside', inside: 'inside', ampaTime: 'AMPA: fast, a few ms', nmdaTime: 'NMDA: slow, tens to hundreds of ms', blockedPct: 'of NMDA channels blocked', ampaNote: 'linear I-V, reversal near 0 mV', nmdaNote: 'the same reversal potential, but the current collapses at negative Vm because of the block', blockNote: 'near rest most channels are plugged; glutamate alone gives little NMDA current', unblockNote: 'depolarization has cleared the pore; Ca2+ enters with Na+' },
        },
        'AMPA versus NMDA. Drag Vm: the AMPA current is a straight line through 0 mV; the NMDA current is tiny near rest and grows as depolarization relieves the Mg2+ block (Jahr and Stevens form). The pore cartoon shows where the Mg2+ is; the lower traces show the fast AMPA and slow NMDA time courses (slide 42).',
        ...
        { type: 'compare', title: 'Four receptors compared', columns: ['Mechanism', 'Ions', 'Speed', 'Function'], rows: [
          { label: 'AMPA', cells: ['Glutamate-gated channel', 'Na+ in, K+ out', 'Fast, ms', 'Fast EPSP; provides the depolarization that unblocks NMDA'] },
          { label: 'NMDA', cells: ['Glutamate-gated channel, Mg2+ block removed by depolarization', 'Na+ in, K+ out, Ca2+ in', 'Slow, tens to hundreds of ms', 'Coincidence detector; Ca2+ signal for lasting change'] },
          { label: 'GABA-A', cells: ['GABA-gated channel', 'Cl-', 'Fast', 'Fast IPSP, shunting inhibition; drug sites'] },
          { label: 'GABA-B', cells: ['G-protein-coupled; G-protein opens K+ channels (shortcut pathway)', 'K+ out', 'Slower, tens of ms and longer', 'Slow, prolonged inhibition toward EK'] },
        ] },
        { type: 'detail', title: 'Excitotoxicity (Box 6.4)', body: 'Too much glutamate kills neurons. When energy fails (stroke, cardiac arrest), membranes depolarize, Ca2+ leaks in, glutamate is released, and NMDA receptors become the main route for a lethal Ca2+ load that activates enzymes which digest the cell. NMDA antagonists are therefore of clinical interest.' },
      ],
      conceptQuiz: [
        {
          id: 'nmda-1',
          prompt: 'Glutamate is applied to a neuron held at -65 mV in normal Mg2+. Why is the NMDA current small?',
          options: [
            { text: 'Mg2+ plugs the open pore at negative potentials', feedback: 'Correct. Depolarization is needed to relieve the block.' },
        ...
        },
        {
          ...
          options: [
            { text: 'GABA-B', feedback: 'Correct. Its G-protein opens K+ channels.' },
            { text: 'GABA-A', feedback: 'GABA-A is itself a Cl- channel.' },
        ...
        {
          id: 'nmda-3',
          prompt: 'What does Ca2+ entering through NMDA channels do that Na+ entering through AMPA channels does not?',
          options: [
            { text: 'Acts as a second messenger that can change enzymes, channels and gene expression', feedback: 'Correct.' },
            { text: 'Depolarizes the membrane', feedback: 'Both do that; the special thing about Ca2+ is its signalling.' },
            { text: 'Repolarizes the membrane', feedback: 'Ca2+ entry depolarizes.' },
          ],
          correct: 0,
        },
      ],
    ...
    // 16 ----------------------------------------------------------------
    {
      ...
      blocks: [
        ...
        { type: 'steps', title: 'Three ways to classify an interneuron (slide 47)', steps: [
          ...
          { title: 'Expression', body: 'of Ca2+-binding proteins (parvalbumin, calbindin, calretinin), neuropeptides (SST, VIP) and receptors (5HT3aR).' },
        ] },
        figureBlock(
        ...
        ),
        {
          ...
          rows: [
            { label: 'PV', cells: ['About 40 percent', 'The Ca2+-binding protein parvalbumin', 'Basket cells, chandelier cells', 'Fast spiking (FS)'] },
      ...
      conceptQuiz: [
        {
          ...
          options: [
            { text: 'PV, fast spiking', feedback: 'Correct.' },
            { text: 'SST', feedback: 'SST includes Martinotti cells, which target dendritic tufts.' },
            { text: '5HT3aR', feedback: 'That group includes VIP and neurogliaform cells.' },
          ],
          correct: 0,
        },
        {
          ...
          options: [
            ...
            { text: 'Serotonin receptor', feedback: 'That is 5HT3aR.' },
            { text: 'A Ca2+-binding protein', feedback: 'Parvalbumin, calbindin and calretinin are the Ca2+-binding proteins.' },
          ],
          correct: 0,
        },
        {
          ...
          options: [
            { text: 'The dendritic tufts of pyramidal cells', feedback: 'Correct.' },
            { text: 'The axon initial segment', feedback: 'That is the chandelier cell.' },
            { text: 'The soma', feedback: 'That is the basket cell.' },
          ],
          correct: 0,
        },
      ],
      ...
      conceptQuiz: [
        {
          ...
          options: [
            { text: 'Interneurons of the same firing type, such as FS with FS', feedback: 'Correct.' },
            { text: 'PV cells with pyramidal cells', feedback: 'PV cells inhibit pyramidal cells chemically.' },
            { text: 'VIP cells with SOM cells', feedback: 'VIP inhibits SOM through GABA synapses, not gap junctions.' },
          ],
          correct: 0,
        },
      ],
    ...
    // 18 ----------------------------------------------------------------
    {
      ...
      blocks: [
        ...
        { type: 'steps', title: 'How a G-protein-coupled receptor works', steps: [
          ...
          'Effector 1, the shortcut pathway: a G-protein-gated ion channel, within 30 to 100 ms, membrane-delimited (muscarinic receptors in the heart; GABA-B).',
        ...
        figureBlock(
          ...
          'Modulation by the NE beta receptor (textbook Figure 5.22, slide 53). Follow the numbers from the receptor to the K+ channel.',
        ...
        { type: 'steps', title: 'The beta-receptor cascade, step by step', steps: [
          ...
          'PKA phosphorylates a type of K+ channel in the dendritic membrane, which closes.',
          'Consequence: lower K+ conductance, higher membrane resistance, longer length constant. Distant or weak EPSPs now reach the spike-initiation zone. NE itself barely changes Vm; it modulates the response to other inputs, and the effect outlasts the NE because of the biochemical intermediaries.',
        ] },
        {
          type: 'compare',
      ...
      conceptQuiz: [
        {
          ...
          options: [
            { text: 'Blocks ACh at nicotinic receptors (an antagonist)', feedback: 'Correct.' },
            { text: 'Activates nicotinic receptors', feedback: 'That is nicotine, an agonist.' },
            { text: 'Blocks acetylcholinesterase', feedback: 'That is what nerve gases do.' },
          ],
          correct: 0,
        },
        {
          ...
          options: [
            ...
            { text: 'The G-protein', feedback: 'The G-protein is the link between receptor and enzyme, not a messenger molecule.' },
            { text: 'Protein kinase A', feedback: 'PKA is the enzyme that cAMP activates.' },
          ],
          correct: 0,
        },
        {
          id: 'pharm-3',
          prompt: 'Closing dendritic K+ channels makes the neuron more excitable because',
          options: [
            { text: 'Membrane resistance and the length constant rise, so distant EPSPs reach the hillock', feedback: 'Correct.' },
            { text: 'It depolarizes the cell to threshold', feedback: 'NE alone causes little change in Vm.' },
            { text: 'It opens Na+ channels', feedback: 'No Na+ channels are involved.' },
          ],
          correct: 0,
        },
      ],
  ...
  recap: {
    terms: [
      ...
      { term: 'Dendritic spine', definition: 'Head on a neck; receives synaptic input; PSD, spine apparatus; shape changes with activity. 3D EM shows contacts, not strength.' },
      ...
      { term: 'Transmitter classes', definition: 'Amino acids: GABA, glutamate, glycine. Amines: DA, epinephrine, histamine, NE, 5-HT (ACh grouped with them). Peptides: CCK, dynorphin, enkephalins, NAAG, NPY, somatostatin, substance P, TRH, VIP.' },
      { term: 'Synthesis', definition: 'Peptides: rough ER precursor, Golgi splits and packs into granules, kinesin transport to terminal. Amines and amino acids: enzymes in terminal cytosol, transporters load vesicles.' },
      { term: 'Release', definition: 'Spike opens voltage-gated Ca2+ channels at the active zone; Ca2+ microdomain; docked vesicles; synaptotagmin senses Ca2+; SNAREs (v on vesicle, t on target) fuse; fusion pore; exocytosis in 0.2 ms (60 us mammal); endocytosis recycles. Botulinum toxin cuts SNAREs.' },
      { term: 'Quantum', definition: 'Contents of one vesicle. CNS synapse about 1 per spike (tenths of a mV); NMJ about 200 (40 mV).' },
      { term: 'Ionotropic receptor', definition: 'Transmitter-gated ion channel, 4 or 5 subunits round a pore, opens in microseconds. Na+-permeable: EPSP (ACh, glutamate). Cl-: IPSP (GABA, glycine).' },
      { term: 'Metabotropic receptor', definition: 'G-protein-coupled, 7 helices. Effectors: G-protein-gated channel (shortcut, 30 to 100 ms) or enzyme making second messengers. Slow, long, diverse, amplified.' },
      ...
      { term: 'Reversal potential', definition: 'Vm where the channel current changes direction (I-V plot). Mixed Na+/K+: about 0 mV. Excitatory if E_rev above threshold, inhibitory if below.' },
      { term: 'EPSP / IPSP', definition: 'Brief depolarization toward threshold / brief hyperpolarization. Inhibition does not need hyperpolarization: at ECl the conductance alone shunts.' },
      { term: 'Summation', definition: 'Spatial: different synapses at once. Temporal: same synapse within 1 to 15 ms. Spike only if threshold at the initiation zone.' },
      { term: 'Length constant', definition: 'Distance for a steady depolarization to fall to 37 percent. Rises with membrane resistance, falls with internal resistance. Active dendritic channels amplify; signals go both ways.' },
      { term: 'Shunting inhibition', definition: 'Cl- channels at ECl near rest lower membrane resistance; Cl- entry opposes depolarization; somatic EPSP suppressed. Potential at the axon hillock is all that counts; PV basket and chandelier cells hold the veto.' },
      { term: 'Glutamate receptors', definition: 'AMPA (Na+, K+; fast EPSP), NMDA (Na+, K+, Ca2+; Mg2+ block removed by depolarization; slow; coincidence detector; Ca2+ signal), kainate. Named after agonists.' },
      { term: 'GABA receptors', definition: 'GABA-A: Cl- channel, fast IPSP; sites for benzodiazepines (frequency), barbiturates (duration), ethanol, neurosteroids. GABA-B: G-protein opens K+ channels, slow.' },
      { term: 'Pyramidal cells / interneurons', definition: 'Pyramidal: glutamate, spiny, long axons. Interneurons: GABA, 20 to 30 percent of cortical neurons. Basket to soma, chandelier to axon initial segment, Martinotti to tufts.' },
      { term: 'Circuit motifs', definition: 'Feedforward: input excites E and an interneuron that inhibits E. Feedback: E recruits interneurons that inhibit E. Disinhibition: VIP inhibits SOM, freeing E. Balance of excitation and inhibition.' },
      { term: 'Interneuron groups', definition: 'PV about 40 percent (basket, chandelier; fast spiking), SST about 30 percent (Martinotti), 5HT3aR about 30 percent (VIP, neurogliaform). Firing: FS, LTS, IS, BST, LS. Gap junctions within a class.' },
      { term: 'Pharmacology', definition: 'ACh nicotinic (nicotine / curare), muscarinic (muscarine / atropine); NE alpha (phenylephrine / phenoxybenzamine), beta (isoproterenol / propranolol); Glu AMPA (AMPA / CNQX), NMDA (NMDA / AP5); GABA-A (muscimol / bicuculline), GABA-B (baclofen / phaclofen); ATP P2X (suramin); adenosine (caffeine).' },
      { term: 'G-protein cycle', definition: 'Alpha, beta, gamma; GDP at rest; receptor swaps GDP for GTP; alpha-GTP and beta-gamma act on effectors; Gs stimulates, Gi inhibits; alpha hydrolyses GTP and rejoins. Kinases phosphorylate, phosphatases reverse.' },
      { term: 'NE beta cascade', definition: '1 NE binds beta receptor, Gs; 2 adenylyl cyclase; 3 ATP to cAMP; 4 protein kinase A; 5 K+ channel phosphorylated and closed. Higher resistance and length constant, more excitable; modulation outlasts NE. Alpha-2 uses Gi instead.' },
    ...
    equations: [
      {
        ...
        expression: 'I_syn = g_syn * (Vm - E_rev)',
        note: 'Zero at the reversal potential. Inward (negative) below E_rev for a cation channel: EPSP. Mixed Na+/K+ channel: E_rev about 0 mV.',
      },
      {
        name: 'Passive cable',
        expression: 'Vx = Vo / e^(x / lambda)\nat x = lambda: Vx = 0.37 Vo',
        note: 'lambda grows with membrane resistance rm and shrinks with internal resistance ri.',
      },
      {
        name: 'Gap junction arithmetic',
        expression: '6 connexins = 1 connexon;  2 connexons = 1 channel;  many channels = 1 gap junction',
        note: '',
      },
    ],
  },
  ...
  lectureQuiz: [
    // Easy ---------------------------------------------------------------
    {
      ...
      options: [
        { text: 'A half-channel made of six connexin subunits, contributed by one cell', feedback: 'Correct. Two connexons make a gap junction channel.' },
        { text: 'A complete gap junction channel', feedback: 'That needs two connexons, one from each cell.' },
        { text: 'One connexin protein', feedback: 'Six connexins make one connexon.' },
        { text: 'The whole plaque of channels', feedback: 'That is the gap junction.' },
      ],
      correct: 0,
      modelAnswer: [
    ...
    },
    {
      ...
      difficulty: 'easy',
      type: 'mc',
      prompt: 'What triggers synaptic vesicles to fuse with the presynaptic membrane?',
      options: [
      ...
      correct: 0,
      modelAnswer: [
        'The action potential depolarizes the terminal and opens voltage-gated Ca2+ channels at the active zone.',
        'Ca2+ enters down a huge gradient; synaptotagmin binds it and triggers SNARE-mediated fusion, exocytosis.',
      ],
    },
    {
      id: 'q03',
      difficulty: 'easy',
      type: 'mc',
      prompt: 'Which receptor is a transmitter-gated ion channel?',
      options: [
        { text: 'The GABA-A receptor', feedback: 'Correct. It is itself a Cl- channel.' },
        { text: 'The GABA-B receptor', feedback: 'GABA-B works through a G-protein.' },
        { text: 'The muscarinic ACh receptor', feedback: 'Muscarinic receptors are G-protein-coupled.' },
        { text: 'The beta-adrenergic receptor', feedback: 'G-protein-coupled; it raises cAMP.' },
      ],
      correct: 0,
      modelAnswer: [
        'Ionotropic receptors are channels: nicotinic ACh, AMPA, NMDA, kainate, GABA-A and glycine receptors.',
        'Metabotropic receptors act through G-proteins: muscarinic, GABA-B, adrenergic, metabotropic glutamate and many others.',
      ],
    },
    {
      id: 'q04',
      difficulty: 'easy',
      type: 'mc',
      prompt: 'Which transmitter mediates most fast excitation in the brain, and which most fast inhibition?',
      options: [
        { text: 'Glutamate; GABA', feedback: 'Correct.' },
        { text: 'GABA; glutamate', feedback: 'The other way round.' },
        { text: 'Acetylcholine; glycine', feedback: 'ACh excites muscle; glycine handles some inhibition, but the main pair in the brain is glutamate and GABA.' },
        { text: 'Norepinephrine; dopamine', feedback: 'These amines mostly modulate through G-protein-coupled receptors.' },
      ],
      correct: 0,
      modelAnswer: [
        'Glutamate acting on AMPA (and NMDA, kainate) channels gives most fast EPSPs.',
        'GABA acting on GABA-A Cl- channels gives most fast IPSPs; glycine mediates most of the rest.',
      ],
    },
    {
      id: 'q05',
      difficulty: 'easy',
      type: 'mc',
      prompt: 'Curare and atropine both block acetylcholine, but at different receptors. Which is which?',
      options: [
        { text: 'Curare blocks nicotinic receptors, atropine blocks muscarinic receptors', feedback: 'Correct.' },
        { text: 'Curare blocks muscarinic receptors, atropine blocks nicotinic receptors', feedback: 'Swapped.' },
        { text: 'Both block nicotinic receptors', feedback: 'Atropine is the muscarinic antagonist.' },
        { text: 'They are agonists, not antagonists', feedback: 'Nicotine and muscarine are the agonists.' },
      ],
      correct: 0,
      modelAnswer: [
        'Nicotinic receptors: agonist nicotine, antagonist curare. Muscarinic receptors: agonist muscarine, antagonist atropine.',
    ...
    // Medium -------------------------------------------------------------
    {
      ...
      modelAnswer: [
        'Presynaptic depolarization is what opens the voltage-gated Ca2+ channels.',
        'Ca2+ entry raises the local concentration at the active zone.',
        'Synaptotagmin, the vesicle\'s Ca2+ sensor, is activated.',
    ...
    },
    {
      ...
      modelAnswer: [
        ...
        'Synaptic vesicles (50 nm) hold amino acid and amine transmitters; secretory granules (100 nm, dense core) hold peptides.',
        'Mitochondria supply ATP.',
        'The active zone is where docked vesicles fuse; the cleft is the 20 to 50 nm gap; the postsynaptic density holds the receptors.',
      ],
    },
    {
      id: 'q08',
    ...
    },
    {
      ...
      difficulty: 'medium',
      type: 'essay',
      prompt: 'A transmitter opens a mixed Na+/K+ channel with a reversal potential near 0 mV in a neuron resting at -65 mV. Explain why an EPSP results, and how opening nearby Cl- channels with ECl near rest can shrink that EPSP even though they cause no visible IPSP.',
      points: 3,
      markScheme: [
        { points: 1, text: 'Uses I = g (Vm - E_rev): at -65 mV with E_rev near 0 the driving force is negative, so net positive current flows in, depolarizing the cell toward 0 mV: an EPSP.' },
        { points: 1, text: 'States that opening Cl- channels at ECl near rest produces no net Cl- current and no IPSP, but raises the membrane conductance (lowers resistance).' },
        { points: 1, text: 'Explains that when the EPSP depolarizes the membrane above ECl, Cl- enters and opposes the depolarization, and the current leaks out before reaching the spike-initiation zone: shunting inhibition.' },
      ],
      modelAnswer: [
        'The synaptic current is I = g (Vm - E_rev). At -65 mV with E_rev about 0 mV the driving force is -65 mV: more Na+ enters than K+ leaves, the net current is inward and the membrane depolarizes toward 0 mV. That transient depolarization is the EPSP.',
        'Cl- channels with ECl at about -65 mV have no driving force at rest, so opening them changes Vm little and no IPSP is seen. But the open channels add conductance and lower the membrane resistance.',
        'As soon as the EPSP lifts Vm above ECl, Cl- flows in and pulls it back down, and the depolarizing current leaks out through the low-resistance membrane. The EPSP that reaches the soma is smaller: shunting inhibition, inhibition without hyperpolarization.',
      ],
    },
    {
      id: 'q10',
      ...
      options: [
        { text: 'I = 2 x (-40 - (-65)) = +50 pA, outward, so the membrane repolarizes toward -65 mV: an IPSP', feedback: 'Correct. Cl- entering is an outward positive current.' },
        { text: 'I = 2 x (-65 - (-40)) = -50 pA, inward, an EPSP', feedback: 'The driving force is Vm - E_rev, not E_rev - Vm.' },
        { text: 'Zero, because GABA is inhibitory', feedback: 'The current is zero only at E_rev, and Vm is 25 mV away from it.' },
        { text: 'I = 2 x (-40) = -80 pA, inward', feedback: 'The driving force is measured from E_rev, not from 0 mV.' },
      ],
      correct: 0,
      modelAnswer: [
        'I = g (Vm - E_rev) = 2 nS x (-40 - (-65)) mV = 2 x 25 = +50 pA.',
        'Positive means outward: Cl- entering the cell is equivalent to positive charge leaving. Vm moves toward ECl, -65 mV, away from threshold: an IPSP.',
    ...
    },
    {
      ...
      difficulty: 'hard',
      type: 'essay',
      prompt: 'Compare the AMPA, NMDA, GABA-A and GABA-B receptors: mechanism, ions, voltage dependence, speed and function. Then predict what happens at a glutamate synapse if the postsynaptic neuron is held at -65 mV while glutamate is released, and if it is depolarized to -30 mV at the same time.',
      points: 6,
      markScheme: [
        { points: 1, text: 'AMPA: glutamate-gated channel passing Na+ and K+, net inward current at negative Vm, fast EPSP within milliseconds.' },
        { points: 1, text: 'NMDA: glutamate-gated channel passing Na+, K+ and Ca2+; Mg2+ blocks the pore at negative Vm and depolarization relieves the block; slower and longer-lasting current.' },
        { points: 1, text: 'GABA-A: GABA-gated Cl- channel; fast IPSP or shunting toward ECl; separate modulatory sites for benzodiazepines, barbiturates, ethanol, neurosteroids.' },
        { points: 1, text: 'GABA-B: G-protein-coupled; the G-protein opens K+ channels (shortcut pathway); slower, longer inhibition toward EK.' },
        { points: 1, text: 'Function: AMPA carries fast excitation and supplies the depolarization; NMDA is a coincidence detector whose Ca2+ entry drives lasting change; GABA-A gives fast inhibition; GABA-B slow inhibition.' },
        { points: 1, text: 'Prediction: at -65 mV glutamate gives an AMPA EPSP but little NMDA current because of the Mg2+ block; at -30 mV the block is relieved, NMDA current adds to the EPSP and Ca2+ enters.' },
      ],
      modelAnswer: [
        'AMPA receptors are glutamate-gated channels permeable to Na+ and K+. At negative membrane potentials more Na+ enters than K+ leaves, so they carry a net inward current and produce the rapid EPSP within a few milliseconds.',
        'NMDA receptors are also glutamate-gated but pass Ca2+ as well as Na+ and K+, and their pore is plugged by Mg2+ at negative potentials. Only depolarization expels the Mg2+, so their current is voltage-dependent, and it rises and decays slowly, over tens to hundreds of milliseconds.',
        'GABA-A receptors are GABA-gated Cl- channels. Opening them pulls Vm toward ECl, about -65 mV: a fast IPSP if Vm is above ECl, and shunting inhibition even when it is not. Benzodiazepines, barbiturates, ethanol and neurosteroids bind separate sites and change the response to GABA.',
        'GABA-B receptors are G-protein-coupled. The activated G-protein opens K+ channels directly, the shortcut pathway, so the inhibition toward EK is slower to start and longer-lasting than the GABA-A response.',
        'In the circuit, AMPA carries the fast excitatory signal and provides the depolarization that NMDA needs; NMDA detects the coincidence of glutamate and depolarization and lets in the Ca2+ that changes enzymes, channels and genes; GABA-A gives fast, spatially precise inhibition; GABA-B gives slow, prolonged inhibition.',
        'At -65 mV, released glutamate opens AMPA channels and gives an EPSP, but the NMDA channels, though glutamate-bound, pass little current because Mg2+ blocks them. At -30 mV the Mg2+ leaves the pore: NMDA current now adds a slow component to the EPSP and Ca2+ enters the spine.',
      ],
    },
    {
      id: 'q13',
    ...
    },
    {
      ...
      difficulty: 'hard',
      type: 'mc',
      prompt: 'Two excitatory synapses each produce a 4 mV EPSP at the soma of a neuron with threshold 10 mV above rest. They fire together and the neuron does not spike; the same two fire together while a VIP interneuron is active and the neuron does spike. Which explanation fits the circuit on slide 48?',
      options: [
        { text: 'The VIP cell inhibited an SOM cell, removing its shunting inhibition of the dendrites, so the two EPSPs now sum to more than 10 mV at the hillock', feedback: 'Correct. Disinhibition raised the effective EPSP size.' },
        { text: 'The VIP cell excited the pyramidal cell directly', feedback: 'VIP cells are GABAergic; they inhibit.' },
        { text: 'The VIP cell is electrically coupled to the pyramidal cell', feedback: 'Gap junctions on the slide join interneurons of the same class, not VIP cells to pyramidal cells.' },
        { text: 'Temporal summation of the two EPSPs occurred only the second time', feedback: 'Both times the inputs fired together; what changed was the inhibition.' },
      ],
      correct: 0,
      modelAnswer: [
        'Two 4 mV EPSPs summing spatially give at most 8 mV, below the 10 mV threshold, and any tonic SOM inhibition on the dendrites shunts them further.',
